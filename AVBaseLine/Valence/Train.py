...
from Dataset import ValenceDataSet, DataPrefetcher
...
def valid_step(net, criterion, valid_images, valid_valences):
    valid_outputs = net(valid_images)
    loss = criterion(valid_outputs, valid_valences)
    return loss.item()
...
if __name__ == "__main__":
    ...
    for epoch in range(Config.EPOCHS):
        ...
        while (train_data is not None) and (not early_stop):
            ...
            # Get Data
            images, valences = train_data['image'], train_data['valence'].view(-1, 1)
            if not (images.is_cuda and valences.is_cuda):
                images, valences = images.to(device), valences.to(device)

            # Train Step
            running_loss += train_step(alex_net, optimizer, criterion, images, valences)
            ...
            if iteration % Config.check_iter == 0 or train_iter_per_epoch==iteration:
                ...
                with torch.no_grad():
                    ...
                    while valid_data is not None:
                        ...
                        # Get Data
                        valid_images, valid_valences = valid_data['image'], valid_data['valence'].view(-1, 1)
                        if not (valid_images.is_cuda and valid_valences.is_cuda):
                            valid_images, valid_valences = valid_images.to(device), valid_valences.to(device)

                        # Valid Step
                        valid_loss += valid_step(alex_net, criterion, valid_images, valid_valences)
