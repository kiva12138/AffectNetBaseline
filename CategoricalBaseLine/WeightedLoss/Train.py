...
def train_step(net, traindataloader, train_iter_per_epoch, optimizer, criterion, writer, lr_scheduler, epoch):
    ...
    while train_data is not None:
        ...
        if iteration % Config.check_iter == 0 or train_iter_per_epoch==iteration:
            print('[%d, %5d] loss: %.3f, lr: %.9f' % (epoch+1, iteration, running_loss_manager.mean, lr_schedule.get_last_lr()[-1]))
...
def valid_step(net, validdataloader, criterion, writer, epoch):
    ...
    with torch.no_grad():
        ...
        while valid_data is not None:
            ...
            valid_outputs = net(valid_images).type(dtype=torch.double)
...
def get_weights():
    with open(Config.manually_annotated_file_mount_json, 'r') as f:
        data = json.load(f)
    data = ast.literal_eval(data)
    fall = max(data.values())
    weights = {}
...
if __name__ == "__main__":
    ...
    weights = torch.DoubleTensor(get_weights()).cuda()
